...
export default function WordsTableRow({ word, onEdit, onDelete, onPlayAudio }: WordsTableRowProps) {
  ...
  return (
    <TableRow data-testid={`word-row-${word.id}`}>
      <TableCell className="font-medium" data-testid="word-cell">
        {word.word}
      </TableCell>
      <TableCell data-testid="translation-cell">{word.translation}</TableCell>
      <TableCell data-testid="phonetic-cell">
        {word.phonetic && <span className="text-muted-foreground">{word.phonetic}</span>}
      </TableCell>
      ...
      <TableCell className="text-muted-foreground text-sm" data-testid="created-at-cell">
